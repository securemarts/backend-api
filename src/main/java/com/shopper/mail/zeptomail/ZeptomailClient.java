...
public class ZeptomailClient {
    ...
    public ZeptomailClient(
            ...
            @Value("${app.mail.from:noreply@shopper.local}") String fromAddress,
            @Value("${app.mail.from-name:Shopper}") String fromName
    ) {
        this.fromAddress = fromAddress != null ? fromAddress.trim() : "noreply@shopper.local";
        this.fromName = fromName != null && !fromName.isBlank() ? fromName.trim() : "Shopper";
